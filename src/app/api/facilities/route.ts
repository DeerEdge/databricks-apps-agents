...
const TRUSTS = ["strong", "partial", "weak"] as const;

// GET /api/facilities?capability=icu&state=Bihar[&trust=strong] — the facility records behind a
// regional aggregate, ordered by trust, each carrying the cited facility text. Evidence drill-in.
// Optional `trust` filters server-side so a level's full set is reachable (not capped out by a
// strong-heavy top-N).
export async function GET(req: Request) {
  try {
    ...
    const trustRaw = (url.searchParams.get("trust") ?? "").toLowerCase();
    const trust = (TRUSTS as readonly string[]).includes(trustRaw) ? trustRaw : null;

    const params = [
      { name: "cap", value: capability, type: "STRING" },
      { name: "state", value: state, type: "STRING" },
    ];
    if (trust) params.push({ name: "trust", value: trust, type: "STRING" });
    ...
    const { rows } = await runSql(
      `SELECT name, city, trust, citation, structured, claim, latitude, longitude
       ...
       WHERE capability = :cap AND upper(trim(state)) = upper(trim(:state))
             AND ${trust ? "trust = :trust" : "trust <> 'none'"}
